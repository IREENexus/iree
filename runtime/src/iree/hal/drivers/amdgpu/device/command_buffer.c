...
static IREE_OCL_GLOBAL iree_hsa_kernel_dispatch_packet_t* IREE_OCL_RESTRICT
...
static inline IREE_OCL_ATTRIBUTE_ALWAYS_INLINE uint16_t
iree_hal_amdgpu_device_make_cmd_packet_header(
    IREE_OCL_GLOBAL const iree_hal_amdgpu_device_cmd_header_t* IREE_OCL_RESTRICT
        cmd_header) {
  ...
  const iree_hsa_fence_scope_t scacquire_fence_scope =
      (cmd_header->flags >> IREE_HAL_AMDGPU_DEVICE_CMD_FLAG_FENCE_ACQUIRE_BIT) &
      0x3;
  const iree_hsa_fence_scope_t screlease_fence_scope =
      (cmd_header->flags >> IREE_HAL_AMDGPU_DEVICE_CMD_FLAG_FENCE_RELEASE_BIT) &
      0x3;
...
static void iree_hal_amdgpu_device_cmd_emplace_barrier(
  ...
  const uint64_t queue_mask = state->execution_queue->size - 1;  // power of two
  IREE_OCL_GLOBAL iree_hsa_barrier_or_packet_t* IREE_OCL_RESTRICT packet =
      state->execution_queue->base_address + (packet_id & queue_mask) * 64;

  // No signals to make this a no-op.
  for (size_t i = 0; i < IREE_OCL_ARRAYSIZE(packet->dep_signal); ++i) {
  ...
  const uint32_t barrier_header =
      iree_hal_amdgpu_device_make_cmd_packet_header(cmd_header);
  ...
  iree_hal_amdgpu_device_atomic_store_explicit(
      ...
      barrier_header, iree_hal_amdgpu_device_memory_order_release,
...
static void iree_hal_amdgpu_device_cmd_commit_cfg_packet(
    ...
    IREE_OCL_GLOBAL const iree_hal_amdgpu_device_kernel_args_t*
        IREE_OCL_RESTRICT kernel_args,
    IREE_OCL_GLOBAL void* IREE_OCL_RESTRICT kernarg_ptr) {
  ...
  packet->workgroup_size[0] = 1;
  packet->workgroup_size[1] = 1;
  packet->workgroup_size[2] = 1;
  packet->reserved0 = 0;
...
void iree_hal_amdgpu_device_command_buffer_enqueue_next_block(
  ...
  IREE_OCL_GLOBAL iree_hsa_queue_t* control_queue =
      state->scheduler->control_queue;
  const uint64_t control_packet_id = iree_hsa_queue_add_write_index(
      control_queue, 1u, iree_hal_amdgpu_device_memory_order_relaxed);
  while (control_packet_id -
             iree_hsa_queue_load_read_index(
                 control_queue, iree_hal_amdgpu_device_memory_order_acquire) >=
         control_queue->size) {
  ...
  while (base_packet_id - iree_hsa_queue_load_read_index(
  ...
  IREE_OCL_GLOBAL iree_hsa_kernel_dispatch_packet_t* IREE_OCL_RESTRICT
      control_packet =
          control_queue->base_address + (control_packet_id & queue_mask) * 64;
  ...
  control_packet->kernarg_address = state->control_kernarg_storage;
  ...
  iree_hal_amdgpu_device_atomic_store_explicit(
      (IREE_OCL_GLOBAL iree_hal_amdgpu_device_atomic_uint32_t*)control_packet,
      control_header_setup, iree_hal_amdgpu_device_memory_order_release,
      iree_hal_amdgpu_device_memory_scope_device);
...
static void iree_hal_amdgpu_device_cmd_fill_buffer_issue(
  ...
  const uint64_t length =
      iree_hal_amdgpu_device_buffer_ref_length(cmd->target_ref);
...
static IREE_OCL_GLOBAL iree_hsa_kernel_dispatch_packet_t*
iree_hal_amdgpu_device_cmd_dispatch_reserve(
    ...
    IREE_OCL_GLOBAL const iree_hal_amdgpu_device_cmd_dispatch_t*
        IREE_OCL_RESTRICT cmd,
    IREE_OCL_GLOBAL uint8_t* IREE_OCL_RESTRICT kernarg_ptr,
    const uint64_t packet_id) {
  // Populate bindings and constants in the reserved kernarg storage.
  for (uint16_t i = 0; i < cmd->config.binding_count; ++i) {
    ((IREE_OCL_GLOBAL uint64_t*)kernarg_ptr)[i] =
        (uint64_t)iree_hal_amdgpu_device_buffer_ref_resolve(cmd->bindings[i],
                                                            state->bindings);
  }
  iree_ocl_memcpy(kernarg_ptr + cmd->config.binding_count * sizeof(void*),
                  cmd->constants,
                  cmd->config.constant_count * sizeof(uint32_t));
  ...
  // hardware command processor stalls until we're done writing.
  IREE_OCL_GLOBAL iree_hsa_kernel_dispatch_packet_t* packet =
      iree_hal_amdgpu_device_cmd_resolve_dispatch_packet(state, packet_id);
  IREE_OCL_GLOBAL const iree_hal_amdgpu_device_kernel_args_t* dispatch_args =
  ...
  if (cmd->config.flags &
    ...
    IREE_OCL_GLOBAL const uint32_t* workgroups_ptr =
        iree_hal_amdgpu_device_workgroup_count_buffer_ref_resolve(
            cmd->config.workgroups_ref, state->bindings);
    packet->grid_size[0] = workgroups_ptr[0];
    packet->grid_size[1] = workgroups_ptr[1];
    packet->grid_size[2] = workgroups_ptr[2];
  } else {
    // Workgroup count is constant.
    packet->grid_size[0] = cmd->config.grid_size[0];
    packet->grid_size[1] = cmd->config.grid_size[1];
    packet->grid_size[2] = cmd->config.grid_size[2];
  }
...
static void iree_hal_amdgpu_device_cmd_dispatch_issue(
  ...
  IREE_OCL_GLOBAL uint8_t* kernarg_ptr =
...
static void iree_hal_amdgpu_device_cmd_dispatch_indirect_dynamic_issue(
  ...
  // manipulate.
  IREE_OCL_GLOBAL uint8_t* IREE_OCL_RESTRICT dispatch_kernarg_ptr =
      state->execution_kernarg_storage + cmd->kernarg_offset +
      IREE_HAL_AMDGPU_DEVICE_WORKGROUP_COUNT_UPDATE_KERNARG_SIZE;
  IREE_OCL_GLOBAL iree_hsa_kernel_dispatch_packet_t* dispatch_packet =
      iree_hal_amdgpu_device_cmd_dispatch_reserve(
  ...
  IREE_OCL_GLOBAL uint64_t* IREE_OCL_RESTRICT update_kernarg_ptr =
      (IREE_OCL_GLOBAL uint64_t*)(state->execution_kernarg_storage +
                                  cmd->kernarg_offset);
  ...
  const iree_hal_amdgpu_device_kernel_args_t update_args =
      state->kernels->cmd_dispatch_indirect_update;
  IREE_OCL_GLOBAL iree_hsa_kernel_dispatch_packet_t* update_packet =
      iree_hal_amdgpu_device_cmd_resolve_dispatch_packet(state, packet_id);
...
static void iree_hal_amdgpu_device_cmd_return_issue(
  ...
  // Emplace and ready the CFG packet.
  const iree_hal_amdgpu_device_kernel_args_t kernel_args =
      state->kernels->cmd_return;
  return iree_hal_amdgpu_device_cmd_commit_cfg_packet(
      state, &cmd->header, packet_id, execution_query_id,
      &state->kernels->cmd_return, kernarg_ptr);
}
