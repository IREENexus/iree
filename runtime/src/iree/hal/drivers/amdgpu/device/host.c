...
void iree_hal_amdgpu_device_host_post(
    IREE_OCL_GLOBAL const iree_hal_amdgpu_device_host_t* IREE_OCL_RESTRICT host,
  ...
  const uint64_t packet_id = iree_hsa_queue_add_write_index(
      host->queue, 1u, iree_hal_amdgpu_device_memory_order_relaxed);
  while (packet_id -
             iree_hsa_queue_load_read_index(
                 host->queue, iree_hal_amdgpu_device_memory_order_acquire) >=
         host->queue->size) {
  ...
  IREE_OCL_GLOBAL iree_hsa_agent_dispatch_packet_t* IREE_OCL_RESTRICT
      agent_packet = host->queue->base_address + (packet_id & queue_mask) * 64;
...
void iree_hal_amdgpu_device_host_post_release(
  ...
  IREE_OCL_TRACE_BUFFER_DEFINE(host->trace_buffer);
...
void iree_hal_amdgpu_device_host_post_retire(
    IREE_OCL_GLOBAL const iree_hal_amdgpu_device_host_t* IREE_OCL_RESTRICT host,
    uint64_t scheduler, uint64_t entry) {
  IREE_OCL_TRACE_BUFFER_DEFINE(host->trace_buffer);
  IREE_OCL_TRACE_ZONE_BEGIN(z0);

  iree_hal_amdgpu_device_host_post(
      host, IREE_HAL_AMDGPU_DEVICE_HOST_CALL_POST_RETIRE,
      /*return_address=*/0, scheduler, entry, 0, 0, iree_hsa_signal_null());
...
void iree_hal_amdgpu_device_host_post_trace_flush(
  ...
  iree_hal_amdgpu_device_host_post(
      ...
      /*return_address=*/0, (uint64_t)host->trace_buffer, /*unused=*/0,
      /*unused=*/0,
